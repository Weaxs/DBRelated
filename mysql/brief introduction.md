...
1. 在执行UPDATE/DELETE的操作时，会把trx_id改为当前事务id 把roll_pointer指向修改前的信息形成版本链
2. 在执行INSERT的操作时，会把trx_id赋值当前事务id，并生成undo日志，但undo日志没有roll_pointer属性，因为它没有修改前的值
3.在执行SELECT的操作时，引入ReadView的概念:
...
* 混合模式复制(mixed-based replication, MBR): 
        ...
        对于STATEMENT模式无法复制的操作使用ROW模式保存binlog，MySQL会根据执行的SQL语句选择日志保存方式。(有可能主从不一致)
...
* 水平拆分:保持数据表结构不变，通过某种策略存储数据分片。这样每一片数据分散到不同的表或者库中，达到了分布式的目的。 
...
* range拆分:每个库一段连续的数据，这个一般是按比如时间范围来的。但是这种一般较少用，因为很容易产生热点问题，大量的流量都打在最新的数据上了。
        ...
        扩容的时候很简单，因为你只要预备好，给每个月都准备一个库就可以了，到了一个新的月份的时候，自然而然，就会写新的库了；
